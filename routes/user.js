...
const SQL_SELECT_ALL = 'SELECT * FROM dbo.user_login for json path;';

// for json path, without_array_wrapper - use for single json result
const SQL_SELECT_BY_ID =
  'SELECT * FROM dbo.user_login WHERE user_id = @id for json path, without_array_wrapper;';
...
router.get(
  ...
  async (req, res) => {
    ...
    if (!validator.isNumeric(user_id, { no_symbols: true })) {
